...
- Domain breakdown: [subDomain].[rootDomain].[topLevelDomain]  
    * example:  
        "startup.mymovieknight.com"  
        "startup" = sub domain  
        "mymovieknight" = root domain  
        ".com" = top level domain (TLD)  

### Manage DNS Records
(DNS = Domain Name System; a DNS A record can point to an IP address or another A record)
...
Web Certificates
- necessary for accessing HTTPS domains
...
**mv** - Move file(s)  
**cp** - Copy files  
**ls** - List files (-l flag: detailed format; -a flag: display hidden files starting with ".")  
...
**wget** - Use HTTP or HTTPS to download files (useful for slow or unstable connections)
...
- Box Model ordering:
    1. Content (with dimensions)
    2. Padding (inherits certain styles like background color)
    3. Border
    4. Margin (only whitespace, not actually part of the element)
